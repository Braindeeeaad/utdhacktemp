...
//Change this to use local csv
async function getFileData(cid) {
    try {
        // Create the signed URL
        const url = await pinata.gateways.createSignedURL({
            cid: cid,  // CID of the file on IPFS
            expires: 30  // URL expiration time in minutes
        });

        // Fetch the file data from the signed URL
        ...
        // Assuming it's a file (e.g., an image or PDF)
        const fileBlob = await response.blob();

        // Example: You can then create an object URL for it, if you want to display it
        const fileUrl = URL.createObjectURL(fileBlob);
        
        console.log("File URL:", fileUrl);
        // You can now use this URL to display the file, download it, or process further.
    } catch (error) {
...
export const getFile = async (uniqueName) => {
    try {
        ...
        const { data, contentType } = await pinata.gateways.get("bafkreiestqxdhjxmkarhxmi7vljtdqybyxht5gui7cub4mwhnate74zrxu");
...
export const isFileInDict = (fileName) => {
    const dict = getDict(); 
