...
import { FiUpload, FiFile, FiTrash2, FiLoader, FiDownload, FiEye } from 'react-icons/fi';
...
function Hero() {
  ...
  const deleteOCRResult = (resultToDelete) => {
    setOcrResults(ocrResults.filter(result => result !== resultToDelete));
  ...
  const downloadCombinedDocument = () => {
    ...
    // preview navigation
    const goToPreview = () => {
        navigate('/preview', { state: { combinedDocument } });
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        <button
            onClick={goToPreview}
            className="fixed bottom-9 right-6 flex items-center px-4 py-2 rounded-md text-white bg-indigo-600 hover:bg-indigo-700"
            >
